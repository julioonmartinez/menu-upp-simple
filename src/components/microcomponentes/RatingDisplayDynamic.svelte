<!-- RatingDisplayDynamic.svelte: Componente principal optimizado -->
<script lang="ts">
  ...
  function formatReviewCount(count: number): string {
    ...
    if (count < 1000) {
      ...
      return (count / 1000).toFixed(1).replace(/\.0$/, '') + 'K reseñas';
    } else {
...
<style>
  ...
  .pill-container:hover {
    background-color: var(--bg-secondary); /* hover:bg-gray-200 */
