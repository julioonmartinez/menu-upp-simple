<script lang="ts">
  //CardDishSvelteCompact.svelte
  import { onMount, createEventDispatcher } from 'svelte';
  import type { CartItem, Dish, DishOption } from '../../interfaces/dish';
  import RatingDisplay from '../microcomponentes/RatingDisplayCompact.svelte';
  ...
  let imageLoaded = false; // Nueva variable para controlar la animación de imagen
  ...
  function handleImageLoad() {
    setTimeout(() => {
      imageLoaded = true;
    }, index * 50); // Delay escalonado basado en el índice
  }
  ...
  <div class="image-container {!item.image || item.image === '' ? 'no-image' : ''}">
    {#if item.image && item.image !== ''}
      <img 
        src={item.image} 
        alt={item.name} 
        class="dish-image {imageLoaded ? 'loaded' : ''}"
        on:load={handleImageLoad}
        on:error={() => console.warn('Error loading image:', item.image)}
      />
    {:else}
...
/* ANIMACIÓN CORREGIDA DE LA IMAGEN */
.dish-image {
  width: 100%;
  height: 100%;
  object-fit: cover;
  transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.4s ease-in-out;
  opacity: 0;
...
.price-container {
  ...
  padding: 0.25rem 0.5rem; /* py-2 px-3 */
  border-radius: 0.75rem; /* rounded-xl */
...
.original-price {
  ...
  color: var(--color-gray-600); /* Cambio de gray-400 a gray-600 para mejor contraste */
...
.discounted-price {
  ...
  font-size: 1.875rem;
  text-shadow: 0 1px 2px rgba(0, 0, 0, 0.1); /* Agregar sombra de texto */
}
